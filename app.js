...
// ─── Inline utility functions (subtitle-utils.js inlined for CI compat) ──────
...
function isFileSizeValid(size) {
    return Number.isFinite(size) && size >= 0 && size <= 2 * 1024 * 1024 * 1024;
}

function parseWhisperChunks(chunks) {
...
function estimateDurationFromWav(buffer) {
    if (!buffer || buffer.byteLength < 44) return 0;
    const view = new DataView(buffer);
    if (view.getUint32(0) !== 0x52494646) return 0;
